...
{
	VertexShader = compile GLSL VS_GENERAL;
	PixelShader = compile GLSL FS_DIRLIGHTING;
	
    ZWriteEnable = false;
	BlendMode = Add;
}

context LIGHTING_CULLNONE
{
	VertexShader = compile GLSL VS_GENERAL;
	PixelShader = compile GLSL FS_LIGHTING;
    // ZEnable = false;
...
void main( void )
{
	...
	// albedo = albedo * textureCube( ambientMap, normal );
...
void main( void )
{
	...
	gl_FragColor.rgb =
		calcPhongSpotLight( newPos, normalize( normal ), albedo.rgb, matSpecParams.rgb,
		                    matSpecParams.a, -vsPos.z, ambientIntensity );
}
...
void main( void )
{
	...
	gl_FragColor.rgb =
		calcPhongDirLight( newPos, normalize( normal ), albedo.rgb, matSpecParams.rgb,
		                    matSpecParams.a, -vsPos.z, ambientIntensity );
}
