...
class Scene(SceneBase):
    ...
    def start_animation(self, content_dir, skip=10) :
...
class SPHDemo(object) :
    ...
    def __base(self, scene, scale, skip=5) :
        self.sph.start_animation(os.path.join(self.__datadir, scene, '{}{}K'.format(scene, scale)), skip)
        horde_window.activateWindow()

    def armadillo71K(self, skip=5) :
        self.__base('armadillo', 71, skip)

    def armadillo102K(self, skip=5) :
        self.__base('armadillo', 102, skip)

    def armadillo242K(self, skip=5) :
        self.__base('armadillo', 242, skip)

    def bunny42K(self, skip=5) :
        self.__base('bunny', 42, skip)

    def bunny112K(self, skip=5) :
        self.__base('bunny', 122, skip)

    def bunny337K(self, skip=5) :
