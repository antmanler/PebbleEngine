...
RenderTargetView::RenderTargetView(QWidget* parent /*= 0*/, Qt::WFlags flags /*= 0*/) : QWidget(parent, flags),
    m_pipelineID(0), m_camera(0), m_scaleFactor(1.0), m_imgData(0), m_imgDepth(0), depth_min_(1.0), depth_max_(0.0)
{
    setFocusPolicy(Qt::ClickFocus);
    setupUi(this);
    connect(m_target, SIGNAL(activated(int)), this, SLOT(changeTarget(int)));
...
void RenderTargetView::setCamera(int cam)
{
    m_target->clear();
    
    m_pipelineID = h3dGetNodeParamI(cam, H3DCamera::PipeResI);
    int target_num = h3dGetResElemCount(m_pipelineID, H3DPipeRes::TargetElem) ;
    
    m_target->blockSignals(true);
    
    for (int target=0; target < target_num; ++target) {
        const char *id = h3dGetResParamStr(m_pipelineID, H3DPipeRes::TargetElem, target, H3DPipeRes::TargetNameStr);
        m_target->addItem(id);
    }
    ...
    if (target_num > 0) {
        m_target->setCurrentIndex(-1);
    }
    
}

void RenderTargetView::changeTarget(int index) 
{
    int target_id = index;
    ...
    int buffers = h3dGetResParamI(m_pipelineID, H3DPipeRes::TargetElem, target_id, H3DPipeRes::TargetColBufNumI) ;
    for (int i=0; i<buffers; ++i)
        m_buffer->addItem(tr("Color Buffer %1").arg(i), i);
    
    bool depthBuf = h3dGetResParamI(m_pipelineID, H3DPipeRes::TargetElem, target_id, H3DPipeRes::TargetHasDepthI) == 1;
    if( depthBuf )
        m_buffer->addItem(tr("Depth Buffer"), 32);
    
    QString toolTip(tr("Depth Buffer: %1").arg(depthBuf ? tr("enabled") : tr("disabled")));
    
    int width = h3dGetResParamI(m_pipelineID, H3DPipeRes::TargetElem, target_id, H3DPipeRes::TargetWidthI) ;
    int height = h3dGetResParamI(m_pipelineID, H3DPipeRes::TargetElem, target_id, H3DPipeRes::TargetHeightI) ;
    float scale = h3dGetResParamF(m_pipelineID, H3DPipeRes::TargetElem, target_id, H3DPipeRes::TargetScaleF, 0);
    
    if (width == 0 && height == 0) {
        width = h3dGetNodeParamI(m_camera, H3DCamera::ViewportWidthI);
        height = h3dGetNodeParamI(m_camera, H3DCamera::ViewportHeightI);
    }
    width *= scale;
    height *= scale;
    ...
    m_buffer->setCurrentIndex(0);
    m_buffer->blockSignals(false);
    
    curr_id_ = m_target->itemText(index);
...
void RenderTargetView::showBuffer(int index)
{
    ...
    if (index >= 0) {
        ...
        h3dGetRenderTargetData(m_pipelineID, qPrintable(curr_id_), bufferIndex, &width, &height, &m_imgDepth, 0, 0);

        unsigned int bufferSize = width * height * m_imgDepth;
        m_imgData = new float[bufferSize];
        if (h3dGetRenderTargetData(m_pipelineID, qPrintable(curr_id_), bufferIndex, &width, &height, &m_imgDepth, m_imgData, bufferSize * sizeof(float))) {
...
QImage RenderTargetView::convertImageData(float *data, unsigned int width, unsigned int height, unsigned int depth, bool ignoreAlpha)
{
    if (depth == 1) {
        ...
        // find min max
        float *temp = data;
        depth_max_ = 0.0, depth_min_ = 1.0;
        for (int y = int(height)-1; y >= 0; y--)
            for (int x = 0; x < int(width); ++x) {
                depth_min_ = qMin(*temp, depth_min_);
                depth_max_ = qMax(*temp, depth_max_);
                temp++;
            }
        
        depth_min_ -= depth_min_*0.01 ;
        float delta = depth_max_ - depth_min_ ;
        // Slow implementation
        for (int y = int(height)-1; y >= 0; y--)
            for (int x = 0; x < int(width); ++x)
                //				img.setPixel(x, y, qMax(qMin(1.0f,*data++) * 255, 0.0f));
                img.setPixel(x, y, ((*data++ - depth_min_)/delta) * 255);
