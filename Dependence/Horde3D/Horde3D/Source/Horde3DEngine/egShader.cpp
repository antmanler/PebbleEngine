...
namespace
{
uint32 gGetOffsetAfterVersionDecl(const std::string &str)
{
//    const char *str = stdstr.c_str();
    ...
    // find #version
    if (str[p] == '#' && str[p+1] == 'v' && str[p+2] == 'e' && str[p+3] == 'r'
        && str[p+4] == 's' && str[p+5] == 'i' && str[p+6] == 'o' && str[p+7] == 'n') {
        p += 7;
        // find end of line
        while (!(str[p] == '\r'||str[p] == '\n')) ++p;
        if (str[p] == '\r' && str[p] == '\n') p+=1;
...
bool ShaderResource::parseFXSection( char *data )
{
	...
	const char *floatnum = "+-0123456789.eE";
        ...
        bool unitFree[6] = {true, true, true, true, true, true};
#else
...
#if defined(PLATFORM_ES2)
                                                if( sampler.texUnit > 5 ) return raiseError( "FX: texUnit exceeds limit", tok.getLine() );
#else
...
void ShaderResource::compileCombination( ShaderContext &context, ShaderCombination &sc )
{
    ...
    string vertCode = getCode( context.vertCodeIdx )->assembleCode();
    string fragCode = getCode( context.fragCodeIdx )->assembleCode();
    _tmpCode0 = vertCode.insert(gGetOffsetAfterVersionDecl(vertCode), _tmpCode0);
    _tmpCode1 = fragCode.insert(gGetOffsetAfterVersionDecl(fragCode), _tmpCode1);
	...
	if( sc.shaderObj != 0 )
	{
		...
		sc.shaderObj = 0;
		sc.customSamplers.clear();
		sc.customUniforms.clear();
	}
