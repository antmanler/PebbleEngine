...
#ifndef WIN32	//hack: bug in powervr
                                    ...
                                    bool hasMips, bool genMips, bool compress, bool sRGB )
{
    ...
    if( _texSlots[0].texObj )
            glBindTexture( _textures.getRef( _texSlots[0].texObj ).type, _textures.getRef( _texSlots[0].texObj ).glObj );

	// Calculate memory requirements
	tex.memSize = calcTextureSize( format, width, height, tex.depth );
...
#ifdef TARGET_ANDROID	// Workaround for Samsung Galaxy driver bug
...
uint32 RenderDevice::getShaderNativeHandler(uint32 shaderId)
{
    RDIShader &shader = _shaders.getRef( shaderId );
    return shader.oglProgramObj;
}
...
uint32 RenderDevice::createRenderBuffer( uint32 width, uint32 height, TextureFormats::List format,
                                         bool depth, uint32 numColBufs, uint32 samples, bool depthUseRenderBuffer )
{
	...
	if( numColBufs > 0 )
	{
		...
		for( uint32 j = 0; j < numColBufs; ++j )
		{
			...
			if (samples > 0 && glExt::IMG_multisampled_render_to_texture )
				glFramebufferTexture2DMultisampleIMG( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, tex.glObj, 0, samples);
			else
...
#if defined (PLATFORM_ES2)
        if (depthUseRenderBuffer) {
            glGenRenderbuffersEXT( 1, &rb.depthBuf );
			glBindRenderbufferEXT( GL_RENDERBUFFER_EXT, rb.depthBuf );
			glRenderbufferStorageEXT( GL_RENDERBUFFER_EXT, _depthFormat, rb.width, rb.height );
			// Attach the renderbuffer
			glFramebufferRenderbufferEXT( GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, rb.depthBuf );
        } else
#endif
...
#if defined(PLATFORM_ES2) && GL_EXT_discard_framebuffer
        // discard unneeded attachment, gain some performance
        // see http://goo.gl/m5Whn
