...
const string PipelineResource::parseStage( XMLNode &node, PipelineStage &stage )
{
	...
	while( !node1.isEmpty() )
	{
		...
		else if( strcmp( node1.getName(), "DrawGeometry" ) == 0 )
		{
			...
			else if( _stricmp( orderStr, "BACK_TO_FRONT_CENTER" ) == 0 ) order = RenderingOrder::BackToFrontCenter;
...
int PipelineResource::getElemCount( int elem )
{
    ...
    case PipelineResData::TargetElem:
        return (int)_renderTargets.size();
	default:
...
int PipelineResource::getElemParamI( int elem, int elemIdx, int param )
{
	...
	case PipelineResData::StageElem:
		...
		break;
    case PipelineResData::TargetElem:
        if( (unsigned)elemIdx < _renderTargets.size() )
        {
            switch( param )
            {
                case PipelineResData::TargetColBufNumI:
                    return _renderTargets[elemIdx].numColBufs;
                case PipelineResData::TargetHasDepthI:
                    return _renderTargets[elemIdx].hasDepthBuf ? 1 : 0;
                case PipelineResData::TargetWidthI:
                {
                    RenderTarget &rt = _renderTargets[elemIdx];
                    uint32 width = ftoi_r( rt.width * rt.scale );
                    return width == 0 ? _baseWidth*rt.scale : width;
	}
                case PipelineResData::TargetHeightI:
                {
                    RenderTarget &rt = _renderTargets[elemIdx];
                    uint32 height = ftoi_r( rt.height * rt.scale );
                    return height == 0 ? _baseHeight*rt.scale : height;
                }
            }
        }
        break;
...
float PipelineResource::getElemParamF( int elem, int elemIdx, int param, int compIdx )
{
    switch( elem )
	{
    case PipelineResData::TargetElem:
        if( (unsigned)elemIdx < _renderTargets.size() )
        {
            switch( param )
            {
            case PipelineResData::TargetScaleF:
                return _renderTargets[elemIdx].scale;
            }
        }
        break;
	}
...
const char *PipelineResource::getElemParamStr( int elem, int elemIdx, int param )
{
    ...
    case PipelineResData::TargetElem:
        if( (unsigned)elemIdx < _renderTargets.size() )
        {
            switch( param )
            {
                case PipelineResData::TargetNameStr:
                    return _renderTargets[elemIdx].id.c_str();
	}
        }
        break;
	}
