...
bool MaterialResource::isOfClass( const string &theClass )
{
    ...
    if( theClass != "" )
    {
        ...
        {
            //theClass2 = string(&theClass[1]);//theClass.substr( 1, theClass.length() - 1);

            int class2_length = theClass.length() - 1;
            if( _class.find( &theClass[1], 0 ) == 0 )
            {
                if( _class.length() == class2_length )//theClass2.length() )
                {
                    return false;
                }
                else
                {
                    if( _class[class2_length/*theClass2.length()*/] == '.' ) return false;
                }
            }
        }
    }
