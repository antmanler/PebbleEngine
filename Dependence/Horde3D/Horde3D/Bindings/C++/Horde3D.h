...
struct H3DNodeFlags
{
	/*	Enum: H3DNodeFlags
		...
		NoLighting     - Excludes scene node from forward light loop rendering
	...
	*/
	enum List
	{
		...
		NoRayQuery = 4,
		NoLighting = 8,
		Inactive = 15  // NoDraw | NoCastShadow | NoRayQuery
	};
};
...
struct H3DMeshParams
{
	/*	Enum: H3DMeshParams
			The available Mesh node parameters values.
	*/
	enum List
	{
		TriangleList = 0,
...
struct H3DLight
{
	/*	Enum: H3DLight
		...
		ShadowNoJitterI	- enable no jitter for shadow generating
	*/
	enum List
	{
		MatResI = 500,
...
/* Function: h3dHandleContextLost
		Invalidates Device objects.
*/
DLL void h3dHandleContextLost();
...
/* Function: h3dAddResourceRef
		Increases user reference count for a resource.
	
	Details:
	
	Parameters:
		sourceRes   - resource handle
*/

DLL void h3dAddResourceRef( H3DRes sourceRes );
...
/* Function: h3dReuploadResource
		Reuploads a resource.
	
	Details:
		This function reuploads device data (useful after OGL context change) if can.
	...
	Returns:
		true if reupload
*/
DLL bool h3dReuploadResource( H3DRes res );
...
/* Function: h3dReleaseUnusedResources
	...
	Parameters:
		type   - type of resource to be released (H3DResTypes::Undefined for all types)
		
	Returns:
		nothing
*/
DLL void h3dReleaseUnusedResources( int type);
