...
ostream &operator<<(ostream &os, vector3f const &v)
{
...
#define lenq(r) (((r).x)*((r).x)+((r).y)*((r).y)+((r).z)*((r).z))
    return fabs(lenq(lhs) - lenq(rhs)) < 1e-15 ;
}
...
void vec3i_to_string(vector3i* v, char* str, int _)
{
...
#if defined(__APPLE__)
void vec3_to_string(vector3* v, char* str, int _)
{
    ...
    if (x == GLG_ZERO)
    {
        if (y == GLG_ZERO)
        {
            if (z != GLG_ZERO)
            {
                ...
                MAT4(m, 0, 0) = c;
                MAT4(m, 1, 1) = c;
                if (z < 0.0F)//¸³Óè²»Í¬µÄÖµ£¬Ê¹µÃÕýÊ±Õë£¬»òÕßË³Ê±ÕëÐý×ª
                {
                    MAT4(m, 0, 1) = s;
                    MAT4(m, 1, 0) = -s;
                }
...
void mat4_invert(matrix4* dst, const matrix4* m)//ÇóÄæ
{
