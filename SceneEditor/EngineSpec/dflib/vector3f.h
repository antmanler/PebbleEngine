...
public:
    ...
    inline vector3f  perpendicular() const {
        vector3f result;
        if (fabs(x) < fabs(y)) {
            if (fabs(x) < fabs(z)) {
                result.x = 1.0f-x*x;
    ...
    inline float distance(const vector3f &v2) const {
        float dx = x - v2.x;
        float dy = y - v2.y;
        float dz = z - v2.z;
    ...
    inline float distanceSquared(const vector3f &v2) const {
        float dx = x - v2.x;
        float dy = y - v2.y;
        float dz = z - v2.z;
    ...
    inline vector3f  crossProduct(const vector3f &v2 ) const {
    ...
    inline vector3f  vmin(const vector3f &v2) const {
        vector3f result;
        result.x = (x<v2.x? x: v2.x);
        result.y = (y<v2.y? y: v2.y);
        result.z = (z<v2.z? z: v2.z);
        return result;
    }

    inline vector3f  vmax(const vector3f &v2) const {
        vector3f result;
        result.x = (x>v2.x? x: v2.x);
        result.y = (y>v2.y? y: v2.y);
        result.z = (z>v2.z? z: v2.z);
    ...
    inline float  maxnorm(const vector3f &v2) const {
        float dx = v2.x-x; if (dx<0) dx=-dx;
        float dy = v2.y-y; if (dy<0) dy=-dy;
        float dz = v2.z-z; if (dz<0) dz=-dz;
        float maxim = max(dx,dy);
        maxim = max(maxim,dz);
    ...
    inline void bound(const vector3f &lower, const vector3f &upper) {
        if (x<lower.x) x = lower.x;
        else if (x>upper.x) x = upper.x;
        if (y<lower.y) y = lower.y;
        else if (y>upper.y) y = upper.y;
        if (z<lower.z) z = lower.z;
        else if (z>upper.z) z = upper.z;
    }
   ...
   return vResult;*/
    ...
    vector3f inline operator / ( const float s ) const {
        vector3f vResult(0.0f, 0.0f, 0.0f);
        float scalar = 1.0f/s;
    ...
    inline bool equals(const vector3f &s) const {
    ...
    inline void set(const vector3f &other) {
        x = other.x;
        y = other.y;
        z = other.z;
    }

    inline float dotProduct(const vector3f &v2 ) const {
        return( x * v2.x + y * v2.y + z * v2.z  );
    }
    inline float dot(const vector3f &v2 ) const {
        return( x * v2.x + y * v2.y + z * v2.z  );
    }
...
struct vector3
{
    ...
    //        return fabs(lenq(this) - lenq(rhs)) < 1e-8 ;
    //    }
...
GLG_INLINE void vec3_cross(vector3* dst, const vector3* v0, const vector3* v1)//²æ³Ë
{
...
GLG_INLINE Real vec3_dot(const vector3* v0, const vector3* v1)//µã³Ë
{
