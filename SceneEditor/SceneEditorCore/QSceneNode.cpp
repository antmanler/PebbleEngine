...
void QSceneNode::setTag(const QString &tag)
{
    if (signalsBlocked())
    {
        m_xmlNode.setAttribute("tag", tag);
        h3dSetNodeParamStr(m_hordeID, H3DNodeParams::TagStr, qPrintable(tag));
    }
    else if (tag!= QSceneNode::tag())
        m_model->undoStack()->push(new QXmlNodePropertyCommand("Set Tag", this, "Tag", tag, -1));
}
