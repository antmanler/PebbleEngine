...
SceneFile* SceneFile::load(const QString &sceneFileName)
{
    ...
    QDomElement sg = scene->m_sceneFileXml.documentElement().firstChildElement("SceneGraph");
    QDomElement pathes = sg.firstChildElement("EnginePath");
    scene->m_sceneGraph = QFileInfo(QDir(pathes.attribute("scenegraphpath", "")), sg.attribute("src", ""));
	return scene;
}
...
QString SceneFile::attachmentPlugInName()
{

    return m_sceneFileXml.documentElement()
            .firstChildElement("Editor")
            .firstChildElement("AttachmentPlugIn")
            .attribute("name");
}
...
void SceneFile::init()
{
    ...
    QDomElement sg = m_sceneFileXml.documentElement().firstChildElement("SceneGraph");
    QDomElement pathes = sg.firstChildElement("EnginePath");
    ...
    QDomElement settings = sg.firstChildElement("EngineConfig");
    ...
    h3dSetOption(H3DOptions::SRGBLinearization,
                 (settings.attribute("sRGBLinearization", "true").compare("TRUE", Qt::CaseInsensitive) == 0 ||
                  settings.attribute("sRGBLinearization", "1").compare("1", Qt::CaseInsensitive) == 0) ? 1.0f : 0.0f);
    h3dSetOption(H3DOptions::MaxAnisotropy, settings.attribute("maxAnisotropy", "4").toFloat());
    h3dSetOption(H3DOptions::SampleCount, settings.attribute("sampleCount", "0").toFloat());
...
{
    QDomElement editor = m_sceneFileXml.documentElement().namedItem("Editor").toElement();
    QString activeCam = editor.namedItem("ActiveCamera").toElement().attribute("name");
...
{
    QDomElement editor = m_sceneFileXml.documentElement().namedItem("Editor").toElement();
    if (editor.namedItem("ActiveCamera").isNull())
        editor.appendChild(m_sceneFileXml.createElement("ActiveCamera"));
    editor.namedItem("ActiveCamera").toElement().setAttribute("name", cameraName);
}
