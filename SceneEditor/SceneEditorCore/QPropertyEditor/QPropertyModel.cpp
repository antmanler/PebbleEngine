...
QVariant QPropertyModel::data ( const QModelIndex & index, int role /*= Qt::DisplayRole*/ ) const
{
    ...
    case Qt::BackgroundRole:
        if (item->isRoot())  return QColor(120,120,120);//QApplication::palette("QTreeView").brush(QPalette::Normal, QPalette::Button).color();
        break;
    };
    return QVariant();
}
...
{
    if (!index.isValid() || deleting_)
        return Qt::ItemIsEnabled;

    Property *item = qobject_cast<Property*>(static_cast<QObject*>(index.internalPointer()));
    // only allow change of value attribute
    if (item && item->metaObject() && item->isRoot())
        return Qt::ItemIsEnabled;
    else if (item && item->metaObject() && item->isReadOnly())
