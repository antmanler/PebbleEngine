...
{
    if( m_propertyObject && m_propertyObject->metaObject() && m_propertyObject->dynamicPropertyNames().contains( objectName().toLocal8Bit() ) )
        return false;
    if (m_propertyObject && m_propertyObject->metaObject() && m_propertyObject->metaObject()->property(m_propertyObject->metaObject()->indexOfProperty(qPrintable(objectName()))).isWritable())
        return false;
    else
        return true;
}
