    ...
    foreach (QString fileName, pluginsDir.entryList(QDir::Files))
    {
        if (QLibrary::isLibrary(fileName))
        {
            ...
            if (plugin)
            {
                ...
                if( tool )
                {
                    registerToolActions(tool->actions());
...
void PlugInManager::setPropertyWidget(QPropertyEditorWidget* widget)
{
    m_propertyEditor = widget;
    emit propertyWidgetChanged(widget);
}
...
void PlugInManager::registerToolActions(const QList<QAction *> &toolActions)
{
    emit toolRegistered(toolActions);
}
