...
{	
    ...
    if (m_rootItem)
    {
        ...
        // Set node name
        const char *name = qPrintable(parentNode->name());
        h3dSetNodeParamStr(root->hordeId(), H3DNodeParams::NameStr, name);
        // Set node tag
        const char *tag = qPrintable(parentNode->tag());
        h3dSetNodeParamStr(root->hordeId(), H3DNodeParams::TagStr, tag);
