...
HordeSceneEditor::HordeSceneEditor(QWidget* parent /*= 0*/, Qt::WFlags flags /*= 0*/) : QMainWindow(parent, flags), 
    m_sceneFile(0), m_glWidget(0)
{
    ...
    QString qstylepath = QApplication::applicationDirPath()+QDir::separator()+
                     #ifdef Q_WS_MAC
                         "../../../"
                     #endif
                         "../../Data" +QDir::separator()+
    ...
    m_styleSystemWatcher->addPath(qstylepath);
    //qDebug("%s", qPrintable(settings.value("Style", "default.qss").toString()));
    loadStyle(qstylepath);
...
QString HordeSceneEditor::getModelFile(const QString &targetPath, QWidget *parent, const QString &caption)
{
    return HordeModelDialog::getModelFile(targetPath, parent, caption);
}
...
void HordeSceneEditor::enterEvent(QEvent *event)
{
    if (m_sceneFile && !m_renderTimer->isActive() ) {
...
void HordeSceneEditor::leaveEvent(QEvent * event)
{
    if (m_sceneFile && m_renderTimer->isActive()
        && m_glWidget && !m_glWidget->hasFullScreenWidget()) {
...
void HordeSceneEditor::sceneCreated()
{
    ...
    if (m_sceneFile) {
        ...
        connect(m_actionFullscreen, SIGNAL(toggled(bool)),
                m_glWidget, SLOT(setFullScreen(bool)));

        connect(m_actionDebugView, SIGNAL(triggered(bool)),
                m_glWidget, SLOT(enableDebugView(bool)));

        connect(m_actionRenderBB, SIGNAL(triggered(bool)),
                m_glWidget, SLOT(setAABBEnabled(bool)));

        connect(m_actionRenderBaseGrid, SIGNAL(triggered(bool)),
                m_glWidget, SLOT(setBaseGridEnabled(bool)));

        connect(m_miscToolBar->m_navigationSpeed, SIGNAL(valueChanged(double)),
                m_glWidget, SLOT(setNavigationSpeed(double)));

        connect(m_actionCollisionCheck, SIGNAL(toggled(bool)),
                m_glWidget, SLOT(setCollisionCheck(bool)));

        connect(m_glWidget, SIGNAL(transformationMode(int)),
                this, SLOT(setTransformationMode(int)));

        connect(m_glWidget, SIGNAL(moveObject(const float, const float, const float)),
                m_nodeWidget, SLOT(moveObject(const float, const float, const float)));

        connect(m_glWidget, SIGNAL(rotateObject(const float, const float, const float)),
                m_nodeWidget, SLOT(rotateObject(const float, const float, const float)));

        connect(m_glWidget, SIGNAL(scaleObject(const float, const float, const float)),
                m_nodeWidget, SLOT(scaleObject(const float, const float, const float)));

        connect(m_glWidget, SIGNAL(logMessages(const QList<QListWidgetItem*>&)),
                this, SLOT(updateLog(const QList<QListWidgetItem*>&)));

        connect(m_glWidget, SIGNAL(statusMessage(const QString&, int)),
                m_statusBar, SLOT(showMessage(const QString&, int)));

        connect(m_glWidget, SIGNAL(nodeSelected(int)),
                m_sceneTreeWidget, SLOT(selectNode(int)));

        connect(m_glWidget, SIGNAL(fullscreenActive(bool)),
                m_actionFullscreen, SLOT(setChecked(bool)));

        connect(m_glWidget, SIGNAL(resized(int, int)),
                m_cameraToolBar, SLOT(viewportResized(int,int)));

        connect(m_sceneTreeWidget, SIGNAL(currentNodeChanged(QXmlTreeNode*)),
                m_glWidget, SLOT(setCurrentNode(QXmlTreeNode*)));

        connect(m_sceneTreeWidget, SIGNAL(nodeAboutToChange()),
                m_glWidget, SLOT(resetMode()));
...
void HordeSceneEditor::saveScene()
{
    ...
    if (!m_sceneFile->save()) QMessageBox::warning(this, tr("Error"), tr("Error writing to file"));
...
bool HordeSceneEditor::closeScene()
{
    ...
    updateMenus();
    // reset window title
    setWindowTitle(tr("Pebble Editor[*]"));
    setWindowModified(false);
    return true;
}
...
{	
    ...
    case 2:
        ...
        break;
    case 3:
        item->setTextColor(QColor("#256611"));
...
void HordeSceneEditor::updateMenus()
{
    ...
    // Toggle all dock widget actions...
    foreach(QAction* action, m_menuScene->actions())
        action->setEnabled(m_sceneFile!=0);
    // fix : enable or disable  "Add Attachment action"
    m_actionAddAttachment->setEnabled( m_sceneFile && m_pluginManager->attachmentPlugIn());
...
void HordeSceneEditor::configure()
{
    ...
    if (dlg.exec() == QDialog::Accepted)
    {
        ...
        QFileInfo style = QApplication::applicationDirPath()+QDir::separator()+
                  #ifdef Q_WS_MAC
                          "../../../"
                  #endif
                          "../../Data/" +QDir::separator()+
                          "Styles"+QDir::separator()+
                          settings.value("Style", "defaultOsx.qss").toString();

        if( style.exists() ) m_styleSystemWatcher->addPath( style.absoluteFilePath() );
        loadStyle( style.absoluteFilePath() );
        settings.endGroup();
        if (m_glWidget) m_glWidget->loadButtonConfig();
    }
}
...
void HordeSceneEditor::fileChanged(const QString& path)
{
    ...
    if (resource != 0)
    {
        ...
        if (h3dGetResType(resource) == H3DResTypes::Pipeline && m_cameraToolBar->currentPipelineID() == resource) {
            ...
            // resize the pipeline
            h3dResizePipelineBuffers(resource, m_glWidget->width(), m_glWidget->height());
        }
#ifdef __APPLE__
...
{		
    ...
    if (file.open(QFile::ReadOnly))
    {
        ...
        QString postfix("");
        if(styleSheet.startsWith(QString("/*icon:white"))) {
            postfix = "W";
        } else if (styleSheet.startsWith(QString("/*icon:color"))) {
            postfix = "C";
        }
        QHordeSceneEditorSettings settings(this);
        settings.beginGroup("General");
        settings.setValue("ThemePostfix", postfix);
        settings.endGroup();
#define ADDICON(name) QIcon(QString(":Icons/%1%2").arg(#name).arg(postfix))
        // change actions icons
        m_actionNew->setIcon(ADDICON(New));
        m_actionSave->setIcon(ADDICON(Save));
        m_actionOpen->setIcon(ADDICON(Open));
        m_actionFullscreen->setIcon(ADDICON(Fullscreen));
        m_actionCopy->setIcon(ADDICON(Copy));
        m_actionPaste->setIcon(ADDICON(Paste));
        m_actionCut->setIcon(ADDICON(Cut));
        m_actionMoveObject->setIcon(ADDICON(Move));
        m_actionRotateObject->setIcon(ADDICON(Rotate));
        m_actionScaleObject->setIcon(ADDICON(Scale));
        m_actionDebugView->setIcon(ADDICON(Wireframe));
        m_actionCameraTransformation2Node->setIcon(ADDICON(Cam2Node));
        m_actionNodeTransformation2Camera->setIcon(ADDICON(Node2Cam));
#undef ADDICON
        m_cameraToolBar->loadStyle(styleSheet);
