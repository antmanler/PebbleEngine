...
HordeModelDialog::HordeModelDialog(const QString& targetPath, QWidget* parent /*= 0*/, Qt::WFlags flags /*= 0*/)
    : HordeFileDialog(H3DResTypes::SceneGraph, targetPath, parent, flags)
    , m_glWidget(0)
    , m_oldScene(0)
    , m_newScene(0)
    , m_currentModel(0)
    , m_cameraID(0)
{
    ...
    m_importButton->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Minimum);
...
{
    // disable rendering in main GLWidget first, this will not loose the curren frame
    ...
    if( scene ) {
        ...
        QDomElement standardPipeline(scene->sceneFileDom().documentElement()
                                     .namedItem("Editor").toElement()
                                     .namedItem("Pipeline").toElement()
                                     );
        int pipelineID = h3dAddResource(H3DResTypes::Pipeline, qPrintable(standardPipeline.attribute("path")), 0);
        SceneEditor::load_h3dres_from_file_qrc(".");
        // pipelineID = h3dGetNodeParamI(HordeSceneEditor::instance()->glContext()->activeCam(), H3DCamera::PipeResI);
        ...
        QDomElement standardLight(scene->sceneFileDom().documentElement()
                                  .namedItem("Editor").toElement()
                                  .namedItem("LightParameters").toElement()
                                  );
    ...
    populateList( settings.value("repositoryDir", DefaultRepoPath.absolutePath()).toString(),
                  QDir( settings.value("repositoryDir", DefaultRepoPath.absolutePath()).toString() ),
                  m_currentFilter, true);
...
void HordeModelDialog::loadModel(const QString& fileName, bool repoFile)
{
	...
	// Load data
    if (envRes == 0 || !SceneEditor::load_h3dres_from_file_qrc(".")) // if loading failed
	{
		// Clear log
		m_xmlView->clear();
		int level;
        ...
        if( repoFile ) QDir::setCurrent( scenePath );
		// Release unused resources within Horde3D
        h3dReleaseUnusedResources(H3DResTypes::Undefined);
		return;
	}
    if (!m_resources.contains(envRes)) m_resources.push_back(envRes);
	// Add to scene graph
