...
#if !defined(__i386__) && defined(_ARM_ARCH_7)//defined(__arm__)
...
static void matmul4_neon(const float *m0, const float *m1, float *d)
{
    asm volatile (
                  "vld1.32 		{d0, d1}, [%1]!			\n\t"	//q0 = m1
                  "vld1.32 		{d2, d3}, [%1]!			\n\t"	//q1 = m1+4
...
public:
        ...
        explicit Vec2f( const Vec2i& val) :
                x( val.x ), y( val.y )
        {
        }
...
#ifdef __MATH_NEON
        matmul4_neon(m1x, m2x, dstx);
        dstx[3] = 0.0f; dstx[7] = 0.0f; dstx[11] = 0.0f; dstx[15] = 1.0f;
#else
